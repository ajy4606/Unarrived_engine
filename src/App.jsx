...
// FRAGMENT SHADER
// 파이프라인 순서:
//   UV 왜곡 → rawRgb 샘플 → 광학 효과 → 색공간 처리 →
//   Zone Shadows/Highlights → ToneMap → SplitToning → Vignette →
//   Contrast/Brightness → 노이즈 생성 → Dither/직접출력 → 합성 효과 → 출력
// ─────────────────────────────────────────────────────────────────────────────
const fragmentShader = `
  ...
  // uIsExporting은 JS 측에서만 사용 (useFrame 가드). GLSL 출력 경로는 동일.
  uniform float uShadows;     // -1~+1, 0=중립
  uniform float uHighlights;  // -1~+1, 0=중립
  uniform float uACESMix;     // 0=없음, 0.5=Reinhard, 1=ACES
  ...
  uniform sampler2D uCurveTex;    // 256×1 RGBA: r=R커브, g=G커브, b=B커브, a=Master커브
  ...
  // toneMap: 0=패스스루, 0→0.5 Reinhard 페이드인, 0.5→1 ACES 페이드인
  // 시뮬레이션: acesMix=0 → 완전 패스스루(색/대비 변형 없음) ✓
  //             acesMix=0.5 → Reinhard 100% (부드러운 롤오프)
  //             acesMix=1.0 → ACES 100% (필름 룩)
  vec3 toneMap(vec3 lin) {
    if (uACESMix < 0.001) return lin;
    float t = min(uACESMix * 2.0, 1.0);           // 0→1 in first half
    float s = clamp(uACESMix * 2.0 - 1.0, 0.0, 1.0); // 0→1 in second half
    vec3 mapped = mix(Reinhard(lin), ACESFilm(lin), s);
    return mix(lin, mapped, t);
  }
  ...
  float patternSample(vec2 uv) {
    ...
    if      (uPatternChannel == 1) return p.r;
    else if (uPatternChannel == 2) return p.g;
    else if (uPatternChannel == 3) return p.b;
    else if (uPatternChannel == 4) return 1.0 - dot(p.rgb, vec3(0.299, 0.587, 0.114));
    else                           return dot(p.rgb, vec3(0.299, 0.587, 0.114));
  }

  vec2 patternGradient(vec2 puv, float eps) {
    return vec2(
      patternSample(puv + vec2(eps, 0.0)) - patternSample(puv - vec2(eps, 0.0)),
      patternSample(puv + vec2(0.0, eps)) - patternSample(puv - vec2(0.0, eps))
    );
  }
  ...
  vec2 voronoiCellUv(vec2 uv, float scale) {
    ...
    for (int dy = -1; dy <= 1; dy++) {
      for (int dx = -1; dx <= 1; dx++) {
        ...
        vec2 center = nb + hash2(nb * 7.3131 + vec2(1.73, 2.91)) * 0.85 + 0.07;
        float d = length(gv - center);
        if (d < minD) { minD = d; bestUv = vec2(center.x / (scale * uAspect), center.y / scale); }
      }
    }
    return bestUv;
  }
  ...
  float halftoneCell(vec2 uv, float luma, float scale, float angle) {
    ...
    vec2 cell = fract(ruv + vec2(floor(ruv.y) * 0.5, 0.0)) - 0.5;
    return step(length(cell), (1.0 - clamp(luma, 0.0, 1.0)) * 0.48);
  }

  float crossHatchCell(vec2 uv, float luma, float scale, float angle) {
    float thick = (1.0 - clamp(luma, 0.0, 1.0)) * 0.5 + 0.04;
    float l1 = fract(dot(uv, vec2(cos(angle), sin(angle))) * scale);
    float l2 = fract(dot(uv, vec2(cos(angle+1.5708), sin(angle+1.5708))) * scale);
    return clamp(step(l1, thick) + step(l2, thick * 0.55), 0.0, 1.0);
  }

  float scatterDot(vec2 bnBase, vec2 nOff, float luma) {
    float bn = texture(uBlueNoise, bnBase + nOff).r * 0.55
             + texture(uBlueNoise, bnBase * 0.4 + nOff + vec2(0.31, 0.67)).r * 0.30
             + texture(uBlueNoise, bnBase * 2.3 + nOff + vec2(0.72, 0.19)).r * 0.15;
    return step(bn, luma);
  }
  ...
  void main() {
    ...
    // ── 1. UV 왜곡 ────────────────────────────────────────────────────────
    // 시뮬레이션: animate=false이면 uEffectTime 정지 → Glitch/Fluidity/Scan 변화 없음 ✓
    if (uGlitch > 0.0) {
    ...
    if (uPatternEnabled && uPatternMode == 0) {
      ...
      modUv += patternGradient(puv, 0.004) * uPatternIntensity * 0.06;
    ...
    if (uCrystallize > 0.0) {
      ...
      modUv = mix(modUv, voronoiCellUv(modUv, exp(mix(log(90.0), log(3.0), eff))), eff);
    }
    if (uScanDisplace > 0.0) {
      ...
      modUv.x     = fract(modUv.x + (rnd - 0.5) * eff * 0.12 + (origL - 0.5) * eff * 0.04);
    }
    if (uThreshCascade > 0.0) {
      float eff    = uThreshCascade * patternMaskValue;
      float origL  = dot(texture(uTexture, modUv).rgb, vec3(0.2126, 0.7152, 0.0722));
      float shadow = 1.0 - smoothstep(0.0, 0.4, origL);
      float hi     = smoothstep(0.6, 1.0, origL);
      modUv = clamp(modUv + vec2((hi - shadow), (shadow - hi) * 0.6) * eff * 0.016, 0.001, 0.999);
    }
    if (uSortGlitch > 0.0) {
      float eff  = uSortGlitch * patternMaskValue;
      float myL  = dot(texture(uTexture, modUv).rgb, vec3(0.299, 0.587, 0.114));
      int   win  = int(eff * 14.0) + 2;
      float maxL = myL; int maxIdx = 0;
      for (int i = 1; i <= 16; i++) {
        if (i > win) break;
        float nbL = dot(texture(uTexture, modUv + vec2(float(i) * uTexelSize.x, 0.0)).rgb, vec3(0.299, 0.587, 0.114));
        if (nbL > maxL) { maxL = nbL; maxIdx = i; }
      }
      float sortMask = smoothstep(0.15, 0.5, myL) * (1.0 - smoothstep(0.65, 0.9, myL));
      modUv.x = mix(modUv.x, fract(modUv.x + float(maxIdx) * uTexelSize.x),
                    eff * sortMask * max(maxL - myL, 0.0) * 3.0);
    }

    vec2  centerDir = modUv - 0.5; centerDir.x *= uAspect;
    ...
    if (uEnableSharpen) {
      vec3 c = rawRgb * 5.0
             ...
             - texture(uTexture, modUv + vec2( uTexelSize.x, 0.0)).rgb
             - texture(uTexture, modUv + vec2(0.0,  uTexelSize.y)).rgb
             - texture(uTexture, modUv + vec2(0.0, -uTexelSize.y)).rgb;
    ...
    if (uFreqPeel > 0.0) {
      ...
      else if (uFreqPeelMode == 1) rawRgb = mix(rawRgb, blurred,       uFreqPeel);
    ...
    if (uEdgeHarvest > 0.0) {
      ...
      edgeMag = clamp(length(vec2(-p00-2.0*p01-p02+p20+2.0*p21+p22,
    ...
    if (uEnableAnamorphic) {
      for (int i = -5; i <= 5; i++) {
        vec3 fs = texture(uTexture, modUv + vec2(float(i) * uFlareWidth, 0.0)).rgb;
        flare += smoothstep(uFlareThreshold, 1.0, dot(fs, vec3(0.2126,0.7152,0.0722)))
               * uFlareColor * (1.0 - abs(float(i)) / 5.0);
      }
    }

    // ── 4. 색공간 처리 (Lift/Gamma/Gain, WB, Saturation) ─────────────────
    // 시뮬레이션: uLinearize=false → rawRgb는 sRGB 입력 그대로 사용
    //             모든 처리는 sRGB 공간에서 일어남
    //             출력단에서 gamma 인코딩 없음 → 브라우저가 그대로 표시 ✓
    ...
    if (uEnableLUT) {
      lin = mix(lin, texture(uLUT3D, clamp(lin, 0.0, 1.0)).rgb, uLUTMix);
    }

    lin = applySaturation(lin, uSaturation);

    // ── 5. Zone Shadows / Highlights ─────────────────────────────────────
    // 시뮬레이션 (shadows=0, highlights=0):
    //   shadowMask * 0 = 0 → 아무 변화 없음 ✓
    //   lin * (1 + 0 * hlMask) = lin → 아무 변화 없음 ✓
    //
    // 시뮬레이션 (shadows=+0.5, highlights=-0.3):
    //   luma=0.1 → shadowMask=pow(0.8,2)=0.64 → lin += 0.5*0.64*0.45 = +0.144 (암부 밝아짐)
    //   luma=0.9 → hlMask=pow(0.8,2)=0.64 → lin *= (1 - 0.3*0.64*0.55) = × 0.894 (하이라이트 복원)
    //   luma=0.5 → shadowMask=0, hlMask=0 → 중간 톤 영향 없음 ✓
    {
      float zoneLuma    = clamp(dot(lin, vec3(0.2126, 0.7152, 0.0722)), 0.0, 1.0);
      float shadowMask  = pow(clamp(1.0 - zoneLuma * 2.0, 0.0, 1.0), 2.0);
      float hlMask      = pow(clamp(zoneLuma * 2.0 - 1.0, 0.0, 1.0), 2.0);
      lin += uShadows    * shadowMask * 0.45;
      lin  = lin * (1.0 + uHighlights * hlMask * 0.55);
    ...
    // Tone Curve: Master 먼저 적용 후 채널별 적용 (Lightroom 방식)
    // 시뮬레이션: uCurveEnabled=false → 패스스루 ✓
    //             identity 커브 → texture(uCurveTex, vec2(x,0.5)).a = x → 변화 없음 ✓
    //             커브 포인트 [(0,0),(128,148),(255,255)] → 중간톤 밝아짐 ✓
    if (uCurveEnabled) {
      // Master curve: a채널 → 각 채널에 동일 적용
      lin.r = texture(uCurveTex, vec2(clamp(lin.r, 0.0, 1.0), 0.5)).a;
      lin.g = texture(uCurveTex, vec2(clamp(lin.g, 0.0, 1.0), 0.5)).a;
      lin.b = texture(uCurveTex, vec2(clamp(lin.b, 0.0, 1.0), 0.5)).a;
      // Per-channel curves: r/g/b 채널
      lin.r = texture(uCurveTex, vec2(clamp(lin.r, 0.0, 1.0), 0.5)).r;
      lin.g = texture(uCurveTex, vec2(clamp(lin.g, 0.0, 1.0), 0.5)).g;
      lin.b = texture(uCurveTex, vec2(clamp(lin.b, 0.0, 1.0), 0.5)).b;
    }

    lin = applySplitToning(lin, dot(lin, vec3(0.2126, 0.7152, 0.0722)));
    lin = clamp(lin, 0.0, 1.5);

    if (uEnableVignette) {
      float maxD = length(vec2(0.5 * uAspect, 0.5));
      lin *= smoothstep(maxD * 0.8 + maxD * 0.5 * uVignetteStrength,
                        max(0.001, maxD * 0.8 - maxD * 0.5 * uVignetteStrength), dist);
      lin  = clamp(lin, 0.0, 1.5);
    }

    // ── 7. Contrast / Brightness → luma / contrastCol ────────────────────
    float luma;
    vec3  contrastCol;
    if (uColorMode) {
    ...
    // ── 8. 노이즈 생성 ────────────────────────────────────────────────────
    // 시뮬레이션: uFrame은 animate=false + temporal=true 시 45프레임까지만 증가 후 정지
    //             → TAA 수렴 후 노이즈 패턴 고정 (정지 시 깨끗한 출력) ✓
    ...
    if (uPatternEnabled && uPatternMode == 2) {
      float pVal = patternSample(patternUv(modUv, uPatternScale, uPatternAngle));
    ...
    // ── 9. Deconstruct 효과 (luma/contrastCol 수정) ───────────────────────
    if (uInkSpread > 0.0) {
      float eff = uInkSpread * patternMaskValue;
      float sD  = eff * 0.006;
      float darkNb = min(min(
        dot(texture(uTexture, modUv + vec2( sD, 0.0)).rgb, vec3(0.299,0.587,0.114)),
        dot(texture(uTexture, modUv + vec2(-sD, 0.0)).rgb, vec3(0.299,0.587,0.114))),min(
        dot(texture(uTexture, modUv + vec2(0.0,  sD)).rgb, vec3(0.299,0.587,0.114)),
        dot(texture(uTexture, modUv + vec2(0.0, -sD)).rgb, vec3(0.299,0.587,0.114))));
    ...
    if (uEdgeHarvest > 0.0) {
      ...
      luma = mix(luma, luma * edgeFade + 0.5 * (1.0 - edgeFade), eff * 0.75);
      if (uColorMode)
        contrastCol = mix(contrastCol, contrastCol * edgeFade + vec3(0.5) * (1.0 - edgeFade), eff * 0.75);
    }
    if (uStipple > 0.0) {
    ...
    // ── 10. Dither / Direct 출력 ──────────────────────────────────────────
    // 시뮬레이션: ditherEnabled=false → contrastCol/luma 그대로 출력 ✓
    vec3 final;
    if (uDitherEnabled > 0.5) {
      ...
      final = uColorMode ? contrastCol : mix(uColor1, uColor2, luma);
    }

    // ── 11. 합성 효과 (Grain, Scatter, Halftone, CrossHatch, Pattern) ─────
    if (uEnableGrain) {
    ...
    if (uScatter > 0.0) {
      vec3 scatterResult = uColorMode
        ? vec3(scatterDot(bnUvBase+vec2(0.11,0.23), nOff, contrastCol.r),
               scatterDot(bnUvBase+vec2(0.47,0.61), nOff, contrastCol.g),
               scatterDot(bnUvBase+vec2(0.83,0.37), nOff, contrastCol.b))
        : mix(uColor1, uColor2, scatterDot(bnUvBase, nOff, luma));
      final = mix(final, scatterResult, uScatter);
    }

    if (uHalftone > 0.0) {
      vec2 auv = modUv * vec2(uAspect, 1.0);
      if (uColorMode) {
        final.r = mix(final.r, mix(0.0, final.r * 1.4, halftoneCell(auv, contrastCol.r, uHalftoneScale, uHalftoneAngle)),         uHalftone);
        final.g = mix(final.g, mix(0.0, final.g * 1.4, halftoneCell(auv, contrastCol.g, uHalftoneScale, uHalftoneAngle+0.2618)), uHalftone);
        final.b = mix(final.b, mix(0.0, final.b * 1.4, halftoneCell(auv, contrastCol.b, uHalftoneScale, uHalftoneAngle+0.5236)), uHalftone);
      } else {
        final = mix(final, mix(uColor2, uColor1, halftoneCell(auv, luma, uHalftoneScale, uHalftoneAngle)), uHalftone);
      }
    }
    ...
    if (uCrossHatch > 0.0) {
      vec2 auv = modUv * vec2(uAspect, 1.0);
      if (uColorMode) {
        final.r = mix(final.r, final.r * mix(0.15, 1.6, crossHatchCell(auv, contrastCol.r, uCrossHatchScale, uCrossHatchAngle)),       uCrossHatch);
        final.g = mix(final.g, final.g * mix(0.15, 1.6, crossHatchCell(auv, contrastCol.g, uCrossHatchScale, uCrossHatchAngle+1.047)), uCrossHatch);
        final.b = mix(final.b, final.b * mix(0.15, 1.6, crossHatchCell(auv, contrastCol.b, uCrossHatchScale, uCrossHatchAngle+2.094)), uCrossHatch);
      } else {
        final = mix(final, mix(uColor2, uColor1, crossHatchCell(auv, luma, uCrossHatchScale, uCrossHatchAngle)), uCrossHatch);
      }
    }

    if (uPatternEnabled && uPatternMode == 3) {
      float pVal = patternSample(patternUv(modUv, uPatternScale, uPatternAngle));
      vec3 soft = mix(2.0 * final * pVal, 1.0 - 2.0 * (1.0 - final) * (1.0 - pVal), step(0.5, pVal));
    ...
    // 출력에서 pow(1/2.2)가 필요 — 단, 이 앱의 주 대상은 sRGB 사진이므로
    // 기본값(uLinearize=false)에서는 인코딩 없음이 정답
    bvec3 isN = isnan(final);
    if (any(isN)) final = vec3(0.0);
...
// blendFragmentShader: TAA 누적 셰이더
// 시뮬레이션 isFinalOutput=true: clamp만 적용, 감마 인코딩 없음 → sRGB 유지 ✓
// 시뮬레이션 isFinalOutput=false: 누적 버퍼로 그대로 저장
const blendFragmentShader = `
  ...
  void main() {
    ...
    gl_FragColor = uIsFinalOutput ? vec4(clamp(blended.rgb, 0.0, 1.0), 1.0) : blended;
  }
`;

// ── 셰이더 머티리얼 ─────────────────────────────────────────────────────────
// DitherMaterial 기본값은 INITIAL_PARAMS와 일치해야 함
// 시뮬레이션: uShadows=0, uHighlights=0 → Zone 조정 없음 ✓
//             uACESMix=0 → toneMap 패스스루 ✓
//             uDitherEnabled=1.0이지만 ditherEnabled=false → useFrame에서 0.0으로 덮어씀 ✓
const DitherMaterial = shaderMaterial(
  {
    uTexture: null, uBlueNoise: null, uLUT3D: null,
    uTime: 0, uEffectTime: 0, uFrame: 0, uAspect: 1.0,
    uDitherEnabled: 0.0,
    uNoiseDensity: 30.0, uNoiseAmount: 0.0,
    ...
    uIsExporting: false,
    uShadows: 0.0, uHighlights: 0.0, uACESMix: 0.0, uLinearize: false,
    ...
    uSplitBalance: 0.0,
    uCrystallize: 0.0, uScanDisplace: 0.0, uEdgeHarvest: 0.0,
    uThreshCascade: 0.0, uStipple: 0.0, uInkSpread: 0.0,
    uSortGlitch: 0.0, uFreqPeel: 0.0, uFreqPeelMode: 0,
    uHalftone: 0.0, uHalftoneScale: 40.0, uHalftoneAngle: 0.0,
    uCrossHatch: 0.0, uCrossHatchAngle: 0.0, uCrossHatchScale: 40.0,
    uScatter: 0.0,
    uPatternTex: null, uPatternEnabled: false,
    uPatternScale: 4.0, uPatternAngle: 0.0, uPatternIntensity: 0.7,
    uPatternChannel: 0, uPatternMode: 0,
    uCurveTex: null, uCurveEnabled: false,
  },
...
// ── EngineCore ───────────────────────────────────────────────────────────────
// 시뮬레이션:
//   animate=false + temporal=true → settleCountRef 45까지 증가 후 렌더 정지 ✓
//   params 변경 → resetTaaFlag=true → 다음 프레임에서 justChanged=true → accumulation=0 강제 ✓
//   temporal=false → currentFbo만 사용, 직접 화면 출력 ✓
const EngineCore = ({ texture, bNoise, lut3D, patternTex, curveTex, params, aspect, saveRef, setIsExporting, setExportResult }) => {
  ...
  const fboOpts = { minFilter: THREE.LinearFilter, magFilter: THREE.LinearFilter, type: THREE.HalfFloatType };
  const targetA    = useFBO(size.width, size.height, fboOpts);
  const targetB    = useFBO(size.width, size.height, fboOpts);
  const currentFbo = useFBO(size.width, size.height, fboOpts);
  ...
  useEffect(() => { resetTaaFlag.current = true; settleCountRef.current = 0; }, [params, texture, size]);
  ...
  useEffect(() => () => { targetA.dispose(); targetB.dispose(); currentFbo.dispose(); }, [targetA, targetB, currentFbo]);

  const uTexelSize = useMemo(() =>
    new THREE.Vector2(1 / (texture.image?.width ?? 1), 1 / (texture.image?.height ?? 1)),
    [texture]
  );
  ...
  useFrame((state, delta) => {
    ...
    } else if (params.temporalEnabled && isDirty) {
      frameRef.current += 1;
    ...
    dm.uCurveTex.value          = curveTex;
    dm.uCurveEnabled.value      = params.curveEnabled;

    if (!params.temporalEnabled) {
      gl.setRenderTarget(currentFbo); gl.render(renderScene, camera);
      ...
      gl.setRenderTarget(null); gl.render(blendScene, orthoCamera);
      return;
    }
    ...
    gl.setRenderTarget(writeBuffer); gl.render(blendScene, orthoCamera);
  ...
  return (
    <>
      <Exporter saveRef={saveRef} params={params}
        setIsExporting={setIsExporting} setExportResult={setExportResult}
        texture={texture} renderScene={renderScene} />
      {createPortal(
        <mesh>
          <planeGeometry args={[planeW, planeH]} />
          <ditherMaterial ref={ditherMatRef}
            uTexture={texture} uBlueNoise={bNoise} uLUT3D={lut3D}
            uPatternTex={patternTex} uCurveTex={curveTex} uAspect={aspect} uTexelSize={uTexelSize} />
        </mesh>, renderScene
      )}
      {createPortal(
        <mesh><planeGeometry args={[2, 2]} /><blendMaterial ref={blendMatRef} /></mesh>,
        blendScene
      )}
    </>
  );
  ...
  useEffect(() => {
    saveRef.current = async () => {
      ...
      renderScene.traverse(o => {
        const u = o.material?.uniforms; if (!u) return;
      ...
      try {
        for (let y = 0; y < outH; y += tileSize) {
          for (let x = 0; x < outW; x += tileSize) {
            ...
            gl.setRenderTarget(rt); gl.render(renderScene, exCam);
            ...
            finalCtx.save(); finalCtx.translate(x, y + ch); finalCtx.scale(1, -1);
            finalCtx.drawImage(tileCanvas, 0, 0); finalCtx.restore();
            await new Promise(r => setTimeout(r, 10));
          }
        }
        setExportResult({ dataUrl: finalCanvas.toDataURL('image/png'), w: outW, h: outH });
      } catch (err) {
        console.error('Export Error:', err);
        alert('이미지 추출 중 오류가 발생했습니다.\n' + err.message);
      } finally {
        exCam.clearViewOffset(); gl.setRenderTarget(null); rt.dispose();
        renderScene.traverse(o => {
          const u = o.material?.uniforms; if (!u) return;
...
// INITIAL_PARAMS: 모든 처리 비활성화 상태 = 원본 이미지와 동일한 출력
// 시뮬레이션: ditherEnabled=false, shadows=0, highlights=0, acesMix=0
//   → 파이프라인 통과 후 최종 출력 ≈ 입력 이미지 ✓
const INITIAL_PARAMS = {
  ...
  acesMix: 0.0, linearize: false,
  colorMode: false,
  saturation: 1.0, colorTemp: 0.0, colorTint: 0.0,
  shadowTint: { ...TINT_NEUTRAL }, highlightTint: { ...TINT_NEUTRAL },
  splitBalance: 0.0,
...
export default function App() {
  ...
  //             zoom=2, container 800×600 → maxX=400, maxY=300 ✓
  const clampPan = useCallback((px, py, z) => {
    if (!canvasContainerRef.current || z <= 1) return { x: 0, y: 0 };
    const { width, height } = canvasContainerRef.current.getBoundingClientRect();
    return {
      x: Math.max(-(width  * (z - 1)) / 2, Math.min((width  * (z - 1)) / 2, px)),
      y: Math.max(-(height * (z - 1)) / 2, Math.min((height * (z - 1)) / 2, py)),
    };
  }, []);
  ...
  // ── 커브 텍스처 빌더 ───────────────────────────────────────────────────────
  // Monotone Cubic (Fritsch-Carlson) 보간: 컨트롤포인트 → 256 LUT
  // 시뮬레이션: points=[[0,0],[255,255]] → 모든 x에서 y=x (identity) ✓
  //             points=[[0,0],[128,180],[255,255]] → 중간톤 밝아짐 (S커브 상단) ✓
  const buildCurveLut = useCallback((points) => {
    const pts = [...points].sort((a, b) => a[0] - b[0]);
    const n = pts.length;
    const lut = new Float32Array(256);

    if (n === 0) { for (let i=0;i<256;i++) lut[i]=i/255; return lut; }
    if (n === 1) { const v=pts[0][1]/255; for (let i=0;i<256;i++) lut[i]=v; return lut; }
    ...
    const dx = [], dy = [], s = [];
    for (let i=0;i<n-1;i++) { dx[i]=xs[i+1]-xs[i]; dy[i]=ys[i+1]-ys[i]; s[i]=dy[i]/dx[i]; }

    // Tangents (endpoint + interior)
    const m = new Array(n);
    m[0] = s[0];
    m[n-1] = s[n-2];
    for (let i=1;i<n-1;i++) {
    ...
    for (let i=0;i<n-1;i++) {
      ...
      const sq=a*a+b*b;
      if (sq>9) { const tau=3/Math.sqrt(sq); m[i]=tau*a*s[i]; m[i+1]=tau*b*s[i]; }
    }

    for (let i=0;i<256;i++) {
      const x = i/255;
      if (x <= xs[0]) { lut[i] = Math.max(0, Math.min(1, ys[0])); continue; }
      if (x >= xs[n-1]) { lut[i] = Math.max(0, Math.min(1, ys[n-1])); continue; }
      let lo=0, hi=n-1;
      while (hi-lo>1) { const mid=(lo+hi)>>1; if (xs[mid]<=x) lo=mid; else hi=mid; }
      const t=(x-xs[lo])/dx[lo], t2=t*t, t3=t2*t;
      const y = (2*t3-3*t2+1)*ys[lo] + (t3-2*t2+t)*dx[lo]*m[lo]
              + (-2*t3+3*t2)*ys[lo+1] + (t3-t2)*dx[lo]*m[lo+1];
      lut[i] = Math.max(0, Math.min(1, y));
    }
    return lut;
  }, []);

  const buildCurveTex = useCallback((curves) => {
    ...
    const data = new Uint8Array(256 * 4); // 256×1 RGBA
    for (let i=0;i<256;i++) {
      data[i*4]   = Math.round(r[i]*255);      // R채널 커브
      data[i*4+1] = Math.round(g[i]*255);      // G채널 커브
      data[i*4+2] = Math.round(b[i]*255);      // B채널 커브
      data[i*4+3] = Math.round(master[i]*255); // Master 커브 (alpha)
    }

    if (curveTexRef.current) {
      // 기존 텍스처 재사용 (dispose 없이 데이터만 갱신)
      curveTexRef.current.image.data.set(data);
      curveTexRef.current.needsUpdate = true;
      return curveTexRef.current;
    }
  ...
  useEffect(() => {
    ...
    loader.load(blueNoiseUrl, (tex) => {
      tex.wrapS = tex.wrapT = THREE.RepeatWrapping;
      tex.minFilter = tex.magFilter = THREE.NearestFilter;
      tex.generateMipmaps = false;
      setBNoise(tex);
    }, undefined, () => alert('🚨 blue-noise.png 로드 실패! src/assets/ 경로를 확인해주세요.'));
  ...
  const parseCubeLUT = useCallback((text) => {
    ...
    for (const raw of lines) {
      ...
      if (/^(DOMAIN_MIN|DOMAIN_MAX|LUT_1D_SIZE|LUT_2D_SIZE)/.test(line)) continue;
  ...
  const handlePatternUpload = useCallback((e) => {
    ...
    new THREE.TextureLoader().load(url, (tex) => {
      ...
      patternRef.current = tex; setPatternTex(tex);
  ...
  useEffect(() => {
    ...
    s.id = id;
    s.textContent = `
      .ua-slider{-webkit-appearance:none;appearance:none;height:3px;border-radius:2px;
      ...
      .ua-slider::-webkit-slider-thumb{-webkit-appearance:none;appearance:none;
        width:12px;height:12px;border-radius:50%;background:#d0d0d0;
        border:1.5px solid #888;cursor:grab;transition:background .1s;}
  ...
  // sidebarContent: useMemo로 메모화 → params/handlers 변경 시만 재생성
  const sidebarContent = useMemo(() => (
    <>
        ...
        {params.halftone > 0 && (
          <div style={{ paddingLeft:10, borderLeft:'2px solid #50c8a033', marginBottom:8 }}>
            ...
            <Sl label="Angle °" min={0} max={180} step={1}
              ...
              onChange={v => sp('halftoneAngle')(v*(Math.PI/180))} accent="#50c8a0" defaultValue={0}/>
          </div>
        )}
        <Sep/>
        ...
        {params.crossHatch > 0 && (
          <div style={{ paddingLeft:10, borderLeft:'2px solid #50c8a033', marginBottom:8 }}>
            ...
            <Sl label="Angle °" min={0} max={180} step={1}
              ...
              onChange={v => sp('crossHatchAngle')(v*(Math.PI/180))} accent="#50c8a0" defaultValue={0}/>
          </div>
        )}
      </Panel>
      ...
      <Panel label="TEXTURE STAMP" accent="#e8a040">
        ...
        <label className="ua-file" style={{ marginBottom:10, display:'block', borderColor: params.patternEnabled ? '#e8a04088' : '#3a3a3a' }}>
            ...
            {params.patternEnabled && (
              <>
                ...
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:4, marginBottom:10 }}>
                  {[['Displace',0,'높이맵 UV 변위'],['Mask',1,'효과 공간 게이팅'],['Threshold',2,'디더 스크린 교체'],['Overlay',3,'Soft Light 합성']].map(([label,v,hint]) => (
                ...
                <Sl label="Angle °" min={0} max={360} step={1}
                  ...
                  onChange={v => sp('patternAngle')(v*(Math.PI/180))} accent="#e8a040" defaultValue={0}/>
                <div style={{ marginTop:8, padding:'7px 10px', background:'#1c1a14', borderRadius:4, border:'1px solid #3a3010', fontSize:10, color:'#9a8060', lineHeight:1.6 }}>
      ...
      {!params.colorMode && (
        <Panel label="DITHER COLORS">
          <div style={{ display:'flex', gap:8 }}>
            <TintSwatch label="Dark tone"  value={params.color1} onChange={e => sp('color1')(e.target.value)} noReset/>
            <TintSwatch label="Light tone" value={params.color2} onChange={e => sp('color2')(e.target.value)} noReset/>
          </div>
        </Panel>
      )}

      <Panel label="TONE CURVE" accent="#7ab8f5">
        <Tog label="Enable Curve" sub="Lightroom식 채널별 톤 커브" checked={params.curveEnabled} onChange={sp('curveEnabled')}/>
        {params.curveEnabled && (
          <CurveEditor
            curves={params.curves}
            onChange={newCurves => setParams(p => ({ ...p, curves: newCurves }))}
            onReset={() => setParams(p => ({
              ...p,
      ...
      <Panel label="TONE MAPPING">
        <div style={{ display:'flex', justifyContent:'space-between', marginBottom:4 }}>
          <Muted>0 = 없음</Muted><Muted>0.5 = Reinhard</Muted><Muted>1 = ACES</Muted>
        </div>
        <Sl label="Curve Mix" min={0} max={1} step={0.01} value={params.acesMix} onChange={sp('acesMix')} defaultValue={0.0}/>
        <Sep/>
        <Tog label="Linear Input" sub="입력이 선형(Log/HDR 렌더)일 때 ON — 일반 sRGB 사진은 OFF" checked={params.linearize} onChange={sp('linearize')}/>
      </Panel>

      <Panel label="EXPOSURE">
        <Sl label="Shadows" min={-1.0} max={1.0} step={0.01} value={params.shadows} onChange={sp('shadows')} defaultValue={0.0}
          sub="← 암부 압축 (crush) · 암부 복원 (lift) →"/>
        <Sl label="Highlights" min={-1.0} max={1.0} step={0.01} value={params.highlights} onChange={sp('highlights')} defaultValue={0.0}
          sub="← 하이라이트 복원 (rolloff) · 하이라이트 증폭 →"/>
      ...
      <Panel label="TEMPORAL" accent="#5a4aaa">
        ...
        <Sep/>
        <Tog label="Temporal Accumulation"
          sub={params.temporalEnabled ? 'ON: 노이즈 누적 안티앨리어싱' : 'OFF: 단일 프레임 출력'}
          checked={params.temporalEnabled} onChange={sp('temporalEnabled')}/>
      ...
      <Panel label="OPTICS &amp; FILM">
        ...
        {params.flare && (<>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:8 }}>
            ...
            <div style={{ position:'relative', width:46, height:22 }}>
              <div style={{ position:'absolute', inset:0, borderRadius:3, background:params.flareColor, border:`1px solid ${C.line2}`, pointerEvents:'none' }}/>
        ...
        {params.caEnable && (<>
          <select value={params.caType} onChange={e => setParams(p => ({...p, caType: parseInt(e.target.value)}))}
            style={{ width:'100%', marginBottom:6, fontSize:11, background:'#252525', color:'#c8c8c8', border:'1px solid #3d3d3d', padding:'5px 8px', borderRadius:3, outline:'none' }}>
      ...
      <Panel label="LUT &amp; PRESETS">
        ...
        <label className="ua-file" style={{ marginBottom:8, display:'block', opacity: webgl2?1:0.4, pointerEvents: webgl2?'auto':'none', borderColor: params.lutEnabled?'#4a90d988':'#3a3a3a' }}>
          <input type="file" accept=".cube,.png,image/png" onChange={handleLUTUpload} style={{ display:'none' }} disabled={!webgl2}/>
          {params.lutName ? `✓  ${params.lutName}${params.lutSize ? ` (${params.lutSize}³)` : ''} — 재업로드` : '＋  LUT 불러오기  (.cube / PNG)'}
        </label>
        {params.lutEnabled && !params.lutDataUrl && params.lutName && (
          <div style={{ fontSize:10, color:'#6a8050', background:'#1a1e14', border:'1px solid #3a4020', borderRadius:4, padding:'5px 8px', marginBottom:8, lineHeight:1.5 }}>
        ...
        {params.lutEnabled && (
          ...
          }} style={{ width:'100%', marginBottom:8, padding:'5px 0', fontSize:10, background:'none', border:'1px solid #4a2020', color:'#8a5050', borderRadius:3, cursor:'pointer', fontFamily:'inherit' }}>✕ LUT 제거</button>
        )}
        <Sep/>
        <div style={{ display:'flex', gap:6, marginBottom:6 }}>
          <input placeholder="프리셋 이름" value={presetName} onChange={e => setPresetName(e.target.value)}
            style={{ flex:1, padding:'5px 8px', background:'#252525', color:'#c8c8c8', border:'1px solid #3d3d3d', fontSize:11, borderRadius:3, outline:'none', fontFamily:'inherit' }}/>
          <button onClick={() => {
            try {
          ...
          }} style={{ padding:'5px 12px', background:'#4a90d9', color:'#fff', border:'none', cursor:'pointer', fontWeight:'600', fontSize:11, borderRadius:3, whiteSpace:'nowrap' }}>저장</button>
        </div>
        <div style={{ maxHeight:110, overflowY:'auto', borderRadius:3, border:'1px solid #2d2d2d', background:'#1c1c1c' }}>
          ...
          {presets.map(p => (
            <div key={p.id} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'7px 12px', borderBottom:'1px solid #252525', cursor:'pointer', transition:'background .1s' }}
              ...
              }} style={{ color:'#804040', fontSize:11, padding:'2px 6px', borderRadius:2, background:'#2a1a1a', cursor:'pointer', marginLeft:8 }}>✕</span>
            </div>
          ))}
        </div>
      </Panel>
      ...
      <Panel label="EXPORT">
        ...
        <div style={{ display:'flex', justifyContent:'flex-end', marginTop:-4, marginBottom:8 }}>
          <span style={{ fontSize:11, color:'#5a7a9a', fontVariantNumeric:'tabular-nums' }}>
            {aspect >= 1 ? `${params.resolution} × ${Math.round(params.resolution/aspect)} px` : `${Math.round(params.resolution*aspect)} × ${params.resolution} px`}
          </span>
        </div>
      </Panel>
    ...
    <div style={{ width:'100vw', height:'100dvh', display:'flex', background:'#141414', color:'#c8c8c8',
      ...
      <div ref={canvasContainerRef}
        ...
        onMouseMove={e => {
          ...
          setPan(clampPan(panStart.current.px + (e.clientX - panStart.current.x),
                          panStart.current.py + (e.clientY - panStart.current.y), zoom));
        }}
        ...
        onTouchMove={e => {
          ...
          e.preventDefault();
          setPan(clampPan(panStart.current.px + (e.touches[0].clientX - panStart.current.x),
                          panStart.current.py + (e.touches[0].clientY - panStart.current.y), zoom));
        }}
        onTouchEnd={() => { isPanning.current = false; setIsPanningState(false); }}
        onWheel={e => {
          e.preventDefault();
          setZoomVisible(true);
          ...
          if (e.deltaY < 0 && cur < LEVELS.length - 1) {
            const next = LEVELS[cur + 1]; setZoom(next); setPan(p => clampPan(p.x, p.y, next));
          } else if (e.deltaY > 0 && cur > 0) {
            const next = LEVELS[cur - 1]; setZoom(next);
...
const Sl = ({ label, sub, min, max, step=0.001, value, onChange, accent, defaultValue, compact }) => {
  ...
  const handleReset = defaultValue !== undefined ? () => onChange(Math.min(max, Math.max(min, defaultValue))) : null;

  if (compact) return (
    <div style={{ marginBottom:6, minWidth:0, display:'flex', alignItems:'center', gap:6 }}>
      <input type="range" min={min} max={max} step={step} value={value} className="ua-slider"
        style={{ '--val':pct, '--acc':color, flex:1 }} onChange={e => onChange(parseFloat(e.target.value))}/>
      <input type="number" value={disp} step={step} className="ua-num"
        ...
        style={{ width:46, background:C.bgDeep, color:C.text, border:`1px solid ${C.line2}`, fontSize:11,
          textAlign:'right', padding:'2px 5px', borderRadius:3, outline:'none', flexShrink:0, fontFamily:'inherit' }}/>
    </div>
  );
  ...
  return (
    <div style={{ marginBottom:8, minWidth:0 }}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:3 }}>
        <span onDoubleClick={handleReset ?? undefined}
          title={handleReset ? `더블클릭 → 초기값 (${step>=1 ? Math.round(defaultValue) : defaultValue})` : undefined}
          style={{ fontSize:11, color:C.text, letterSpacing:'-0.1px', overflow:'hidden', textOverflow:'ellipsis',
            whiteSpace:'nowrap', flex:1, marginRight:6, cursor: handleReset ? 'pointer' : 'default', userSelect:'none' }}>
          {label}
        </span>
        <input type="number" value={disp} step={step} className="ua-num"
          onChange={e => { const v = parseFloat(e.target.value); if (!isNaN(v)) onChange(Math.min(max, Math.max(min, v))); }}
      ...
      <input type="range" min={min} max={max} step={step} value={value} className="ua-slider"
        style={{ '--val':pct, '--acc':color }} onChange={e => onChange(parseFloat(e.target.value))}/>
...
const Tog = ({ label, sub, checked, onChange }) => (
  <label className="ua-toggle-row" style={{ display:'flex', alignItems:'flex-start', gap:8, cursor:'pointer',
    marginBottom:8, userSelect:'none', color: checked ? C.text : C.textDim }}>
    <span style={{ flexShrink:0, marginTop:1, width:14, height:14, borderRadius:3,
      background: checked ? C.accent : C.bgDeep, border:`1px solid ${checked ? C.accentHi : C.line2}`,
...
const ModeBtn = ({ active, onClick, children }) => (
  <button onClick={onClick} style={{ flex:1, padding:'7px 0',
    background: active ? C.accent : C.bgDeep, color: active ? '#fff' : C.textDim,
    border:`1px solid ${active ? C.accentHi : C.line2}`, borderRadius:4,
    fontWeight:'600', fontSize:11, cursor:'pointer', letterSpacing:'0.04em', transition:'all 0.12s' }}>
    {children}
  </button>
);
...
const TintSwatch = ({ label, value, onChange, onReset, noReset }) => (
  <div style={{ flex:1 }}>
    ...
    <div style={{ position:'relative', height:28 }}>
      <div style={{ position:'absolute', inset:0, borderRadius:4, background:value, border:`1px solid ${C.line2}`, pointerEvents:'none' }}/>
      <input type="color" value={value} onChange={onChange}
        style={{ position:'absolute', inset:0, opacity:0, cursor:'pointer', width:'100%', height:'100%', border:'none', padding:0 }}/>
    </div>
    {!noReset && (
      <button onClick={onReset} style={{ width:'100%', marginTop:4, padding:'3px 0', fontSize:10,
        background:C.bgDeep, color:C.textDim, border:`1px solid ${C.line}`, cursor:'pointer', borderRadius:3, fontFamily:'inherit' }}>Reset</button>
    )}
  </div>
);
...
const EffectRow = ({ name, tag, children, onReset }) => (
  <div style={{ marginBottom:10 }}>
    <div style={{ display:'flex', justifyContent:'space-between', alignItems:'baseline', marginBottom:3 }}>
      <span onDoubleClick={onReset ?? undefined}
        title={onReset ? '더블클릭 → 초기값으로 리셋' : undefined}
        style={{ fontSize:11, color:C.text, fontWeight:'600', cursor: onReset ? 'pointer' : 'default', userSelect:'none' }}>
        {name}
      </span>
...
//
// 시뮬레이션:
//   - identity 커브 [[0,0],[255,255]] → 대각선 직선, 화면 변화 없음 ✓
//   - 포인트 추가 → 클릭 위치에 점 생성, monotone cubic 재보간 ✓
//   - 포인트 드래그 → points 배열 갱신 → buildCurveTex 재빌드 → needsUpdate ✓
//   - x=0/255 앤드포인트 → y만 이동 (x 고정) ✓
//   - 더블클릭 포인트 → 삭제 (단, 앤드포인트는 삭제 불가) ✓
//   - 채널 탭 전환 → activeChannel만 변경, points는 별도 유지 ✓
//
// 설계:
//   - Canvas 2D: 그리드 + 히스토그램 배경 + 다채널 미리보기 + 현재 채널 커브 + 컨트롤 포인트
//   - monotone cubic spline (Fritsch-Carlson) - 오버슈팅 없는 부드러운 커브
//   - 256개 LUT → DataTexture 갱신 (매 dragging 마다 onChange 호출)
//   - 모바일 touch 이벤트 동일 지원

const CURVE_CHANNELS = {
  master: { label: 'M', color: '#c8c8c8', darkColor: '#444444', key: 'master' },
  r:      { label: 'R', color: '#ff6060', darkColor: '#5a2020', key: 'r' },
  g:      { label: 'G', color: '#50d050', darkColor: '#1a4a1a', key: 'g' },
  b:      { label: 'B', color: '#5090ff', darkColor: '#1a2a5a', key: 'b' },
};

// 커브 에디터 내부 LUT 계산 (App의 buildCurveLut과 동일 알고리즘, 독립 구현)
function computeCurveLut(points) {
  ...
  for (let i=0;i<n-1;i++){
    if(Math.abs(s[i])<1e-10){m[i]=m[i+1]=0;continue;}
    const a=m[i]/s[i],b=m[i+1]/s[i],sq=a*a+b*b;
    if(sq>9){const tau=3/Math.sqrt(sq);m[i]=tau*a*s[i];m[i+1]=tau*b*s[i];}
  }
...
const CurveEditor = ({ curves, onChange, onReset }) => {
  const [activeCh,    setActiveCh]    = useState('master');
  ...
  const CW = 220, CH = 180;
  const PAD = 14;
  const PW = CW - PAD*2; // plot width
  const PH = CH - PAD*2; // plot height
  const HIT = 9;         // 히트 반경 (px)
  ...
  const findPoint = useCallback((px, py) => {
    for (let i=0; i<activePts.length; i++) {
      const [cpx, cpy] = valToPx(activePts[i][0], activePts[i][1]);
      if (Math.hypot(px-cpx, py-cpy) <= HIT) return i;
    }
    return -1;
  }, [activePts, valToPx, HIT]);
  ...
  useEffect(() => {
    ...
    ctx.setLineDash([4, 4]);
    ctx.beginPath();
    ctx.moveTo(...valToPx(0, 0));
    ctx.lineTo(...valToPx(255, 255));
    ctx.stroke();
    ctx.setLineDash([]);

    // 비활성 채널 커브 (흐리게)
    Object.entries(CURVE_CHANNELS).forEach(([key, ch]) => {
      if (key === activeCh) return;
      const pts = curves[key];
      if (!pts || pts.length < 2) return;
      const lut = computeCurveLut(pts);
      ctx.beginPath();
      ctx.strokeStyle = ch.darkColor;
      ctx.lineWidth = 1;
      for (let i=0; i<256; i++) {
        const [px, py] = valToPx(i, lut[i]*255);
    ...
    // 활성 채널 커브 (밝게)
    const lut = computeCurveLut(activePts);
    // 커브 아래 영역 채우기 (미묘한 그라디언트)
    const grad = ctx.createLinearGradient(0, PAD, 0, PAD+PH);
    const chColor = CURVE_CHANNELS[activeCh].color;
    grad.addColorStop(0, chColor + '18');
    grad.addColorStop(1, chColor + '04');
    ctx.beginPath();
    ctx.moveTo(...valToPx(0, lut[0]*255));
    for (let i=1; i<256; i++) ctx.lineTo(...valToPx(i, lut[i]*255));
    ...
    // 컨트롤 포인트
    activePts.forEach(([x, y], idx) => {
      const [px, py] = valToPx(x, y);
      const isEnd = (x === 0 || x === 255);
      const isHov = idx === hoveredIdx;
      const isDrg = idx === draggingIdx;

      // 외곽 원
      ctx.beginPath();
      ctx.arc(px, py, isDrg ? 6 : isHov ? 5.5 : 4.5, 0, Math.PI*2);
      ctx.fillStyle = isDrg ? '#fff' : isHov ? '#e8e8e8' : '#c8c8c8';
      ctx.fill();

      // 내부 색상 (채널 색)
      ctx.beginPath();
      ctx.arc(px, py, isDrg ? 3.5 : isHov ? 3 : 2.5, 0, Math.PI*2);
      ctx.fillStyle = isEnd ? '#555' : chColor;
      ctx.fill();
    });
    ...
    [[0,'0'],[127,'½'],[255,'']].forEach(([v, label]) => {
      const [px] = valToPx(v, 0);
  ...
  const handleDown = useCallback((e) => {
    ...
    // 빈 곳 클릭 → 포인트 추가
    const [vx, vy] = pxToVal(px, py);
    // x 중복 체크 (±2 이내면 추가 안 함)
    const tooClose = activePts.some(p => Math.abs(p[0]-vx) <= 2);
    if (!tooClose) {
      const newPts = [...activePts, [vx, vy]].sort((a,b) => a[0]-b[0]);
      onChange({ ...curves, [activeCh]: newPts });
      const newIdx = newPts.findIndex(p => p[0]===vx && p[1]===vy);
      setDraggingIdx(newIdx);
    }
  }, [getCanvasPos, findPoint, pxToVal, activePts, activeCh, curves, onChange]);
  ...
  const handleMove = useCallback((e) => {
    ...
    // 호버 업데이트
    if (draggingIdx === null) {
      setHoveredIdx(findPoint(px, py));
      return;
    }

    const [vx, vy] = pxToVal(px, py);
    const oldPt = activePts[draggingIdx];
    const isEnd = (oldPt[0] === 0 || oldPt[0] === 255);

    // 앤드포인트는 x 고정, 중간 포인트는 이웃과 겹치지 않게 제한
    let newX = isEnd ? oldPt[0] : vx;
    if (!isEnd) {
      const prevX = draggingIdx > 0 ? activePts[draggingIdx-1][0] : -1;
      const nextX = draggingIdx < activePts.length-1 ? activePts[draggingIdx+1][0] : 256;
      newX = Math.max(prevX+1, Math.min(nextX-1, vx));
    }
    const newY = Math.max(0, Math.min(255, vy));
  ...
  const handleDblClick = useCallback((e) => {
    ...
    // 앤드포인트 삭제 불가
    if (pt[0]===0 || pt[0]===255) return;
    const newPts = activePts.filter((_,i) => i!==idx);
    onChange({ ...curves, [activeCh]: newPts });
  }, [getCanvasPos, findPoint, activePts, activeCh, curves, onChange]);
  ...
  return (
    <div style={{ userSelect:'none' }}>
      ...
      <div style={{ display:'flex', gap:3, marginBottom:6 }}>
        {Object.entries(CURVE_CHANNELS).map(([key, ch]) => {
          const isActive = activeCh === key;
          const hasEdit = curves[key] && !(
            curves[key].length === 2 &&
            curves[key][0][0]===0 && curves[key][0][1]===0 &&
            curves[key][1][0]===255 && curves[key][1][1]===255
          );
          return (
            <button key={key} onClick={() => setActiveCh(key)} style={{
              flex:1, padding:'4px 0', fontSize:10, fontWeight:'600',
              borderRadius:3, cursor:'pointer',
              background: isActive ? ch.darkColor : C.bgDeep,
              color: isActive ? ch.color : C.textDim,
              border: `1px solid ${isActive ? ch.color+'66' : C.line2}`,
              position:'relative',
            }}>
              {ch.label}
              {hasEdit && (
                <span style={{
                  ...
                  borderRadius:'50%', background: ch.color, opacity:0.8,
                }}/>
              )}
            </button>
          );
      ...
      <div ref={containerRef} style={{
        ...
        cursor: draggingIdx !== null ? 'none' : hoveredIdx !== null ? 'grab' : 'crosshair',
      }}>
        <canvas
          ref={canvasRef}
          width={CW} height={CH}
        ...
        <div style={{
          ...
          color: CURVE_CHANNELS[activeCh].color, opacity:0.7, letterSpacing:'0.06em',
          pointerEvents:'none',
        }}>
          {CURVE_CHANNELS[activeCh].label === 'M' ? 'MASTER' : CURVE_CHANNELS[activeCh].label.toUpperCase()}
        </div>
      </div>

      {/* 사용 안내 + 리셋 */}
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginTop:5 }}>
        <span style={{ fontSize:9, color:C.textMute, lineHeight:1.4 }}>
          클릭: 포인트 추가 · 더블클릭: 삭제
        </span>
        <div style={{ display:'flex', gap:4 }}>
          <button onClick={() => {
            // 현재 채널만 리셋
            onChange({ ...curves, [activeCh]: [[0,0],[255,255]] });
          }} style={{
            padding:'2px 7px', fontSize:9, borderRadius:2, cursor:'pointer',
            background:C.bgDeep, color:C.textDim, border:`1px solid ${C.line2}`,
            fontFamily:'inherit',
          }}>채널 초기화</button>
          <button onClick={onReset} style={{
            padding:'2px 7px', fontSize:9, borderRadius:2, cursor:'pointer',
            background:C.bgDeep, color:'#7a4a4a', border:'1px solid #3a2a2a',
            fontFamily:'inherit',
          }}>전체 초기화</button>
        </div>
      </div>
